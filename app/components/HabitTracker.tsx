...
function StatsTab({ habits, dark }: { habits: Habit[]; dark: boolean }) {
  ...
  function countForDay(d: string) { return filtered.filter(h => h.completedDays.includes(d)).length; }
  ...
  function YearView() {
    ...
    const totalDone = filtered.reduce((s, h) => s + h.completedDays.filter(d => d.startsWith(`${year}`)).length, 0);
    return (
      <div>
        <div className={`${card} rounded-2xl p-4 mb-3`}>
            ...
            <div style={{ minWidth: weeks.length * 13 }}>
              <div className="flex mb-0.5 ml-7">
                {weeks.map((_, wi) => {
                  ...
                  return <div key={wi} style={{ width: 11, marginRight: 2, flexShrink: 0 }} className={`text-[9px] ${sub} truncate`}>{lbl?.label ?? ""}</div>;
                })}
              </div>
              <div className="flex gap-0.5">
                <div className="flex flex-col gap-0.5 mr-1">
                  {DAYS_SHORT.map((d, i) => (
                    <div key={d} style={{ height: 11 }} className={`text-[9px] ${sub} flex items-center`}>{i % 2 === 0 ? d : ""}</div>
                  ))}
                </div>
                {weeks.map((week, wi) => (
          ...
          <div className="flex items-center gap-1 mt-2 justify-end">
            ...
            {[0, 0.4, 0.7, 1].map((op, i) => (
              <div key={i} style={{ width: 10, height: 10, borderRadius: 2, backgroundColor: op === 0 ? (dark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.06)") : `rgba(181,132,90,${op})` }}/>
            ))}
              ...
              return (
                <div key={m} className="flex items-center gap-2">
                  ...
                  <div className={`flex-1 h-5 rounded-lg overflow-hidden ${dark ? "bg-[#2a2a2a]" : "bg-[#f0e8e0]"}`}>
                    {!isFuture && <div className="h-full rounded-lg bg-[#B5845A] transition-all duration-500" style={{ width: `${pct}%` }}/>}
                  </div>
                  <span className={`text-xs w-8 text-right flex-shrink-0 ${isFuture ? "opacity-30" : ""} ${sub}`}>{isFuture ? "—" : count}</span>
                </div>
              );
            })}
          </div>
  ...
  function MonthView() {
    ...
    const monthDone = filtered.reduce((s, h) => s + h.completedDays.filter(d => d.startsWith(`${year}-${String(month+1).padStart(2,"0")}`)).length, 0);
    return (
      <div className={`${card} rounded-2xl p-4`}>
        <div className="flex items-center justify-between mb-4">
          <button onClick={() => setMonthOffset(o => o - 1)} className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${dark ? "hover:bg-[#2a2a2a] text-zinc-400" : "hover:bg-zinc-100 text-zinc-500"}`}>
          ...
          <button onClick={() => setMonthOffset(o => o + 1)} disabled={monthOffset >= 0} className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors disabled:opacity-30 ${dark ? "hover:bg-[#2a2a2a] text-zinc-400" : "hover:bg-zinc-100 text-zinc-500"}`}>
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7"/></svg>
          </button>
        </div>
        <div className="grid grid-cols-7 mb-1">
          {DAYS_SHORT.map(d => <div key={d} className={`text-center text-[10px] font-medium pb-1 ${sub}`}>{d}</div>)}
        </div>
          ...
          {cells.map((day, i) => {
            ...
            return (
              <button key={i} onClick={() => { setSelectedDay(dateStr); setView("day"); }}
                className={`aspect-square rounded-lg flex flex-col items-center justify-center transition-all hover:opacity-80 ${isToday ? "ring-2 ring-[#B5845A]" : ""}`}
                style={{ backgroundColor: isFuture ? "transparent" : heatColor(count, maxPossible()) }}>
                <span className={`text-xs font-medium ${isFuture ? (dark ? "text-zinc-700" : "text-zinc-300") : pct > 0.5 ? "text-white" : txt}`}>{day}</span>
                {!isFuture && count > 0 && <span className={`text-[8px] ${pct > 0.5 ? "text-white/70" : sub}`}>{count}/{maxPossible()}</span>}
              </button>
            );
          })}
        </div>
      ...
      <div>
        <div className={`${card} rounded-2xl p-4 mb-3`}>
          <div className="flex items-center justify-between">
            <button onClick={() => changeDay(-1)} className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${dark ? "hover:bg-[#2a2a2a] text-zinc-400" : "hover:bg-zinc-100 text-zinc-500"}`}>
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7"/></svg>
            </button>
            <p className={`text-sm font-semibold capitalize ${txt}`}>{dateLabel}</p>
            <button onClick={() => changeDay(1)} disabled={selectedDay >= today()} className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors disabled:opacity-30 ${dark ? "hover:bg-[#2a2a2a] text-zinc-400" : "hover:bg-zinc-100 text-zinc-500"}`}>
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7"/></svg>
            </button>
          </div>
        </div>
        {filtered.length === 0 && <div className={`${card} rounded-2xl p-8 text-center`}><p className={`text-sm ${sub}`}>Нет привычек</p></div>}
        {doneFull.length > 0 && (
          <div className={`${card} rounded-2xl mb-3 overflow-hidden`}>
            <div className={`px-4 py-3 border-b ${dark ? "border-[#2a2a2a]" : "border-zinc-100"}`}><p className={`text-xs font-semibold ${sub}`}>✅ Выполнено ({doneFull.length})</p></div>
            {doneFull.map((h, i) => (
              <div key={h.id} className={`flex items-center gap-3 px-4 py-3 ${i < doneFull.length - 1 ? `border-b ${dark ? "border-[#2a2a2a]" : "border-zinc-100"}` : ""}`}>
                ...
                <div className="flex-1 min-w-0"><span className={`text-sm ${txt}`}>{h.name}</span><p className={`text-xs ${sub}`}>Цель: {h.weeklyGoal === 7 ? "каждый день" : `${h.weeklyGoal}× в неделю`}</p></div>
                <div className="w-6 h-6 rounded-full bg-[#B5845A] flex items-center justify-center flex-shrink-0"><svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"/></svg></div>
              </div>
            ))}
          </div>
        )}
        {notDone.length > 0 && selectedDay <= today() && (
          <div className={`${card} rounded-2xl overflow-hidden`}>
            <div className={`px-4 py-3 border-b ${dark ? "border-[#2a2a2a]" : "border-zinc-100"}`}><p className={`text-xs font-semibold ${sub}`}>❌ Не выполнено ({notDone.length})</p></div>
            {notDone.map((h, i) => (
              <div key={h.id} className={`flex items-center gap-3 px-4 py-3 ${i < notDone.length - 1 ? `border-b ${dark ? "border-[#2a2a2a]" : "border-zinc-100"}` : ""}`}>
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg flex-shrink-0 ${dark ? "bg-[#2a2a2a]" : "bg-[#f4f0ec]"}`}>{h.emoji}</div>
                <div className="flex-1 min-w-0"><span className="text-sm text-zinc-400">{h.name}</span><p className={`text-xs ${sub}`}>Цель: {h.weeklyGoal === 7 ? "каждый день" : `${h.weeklyGoal}× в неделю`}</p></div>
    ...
    <div>
      {habits.length > 1 && (
        <div className="mb-3 flex gap-1.5 flex-wrap">
          <button onClick={() => setFilterHabit("all")} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${pill(filterHabit === "all")}`}>Все привычки</button>
          {habits.map(h => (
            <button key={h.id} onClick={() => setFilterHabit(h.id)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors flex items-center gap-1 ${pill(filterHabit === h.id)}`}>{h.emoji} {h.name}</button>
          ))}
        </div>
      )}
      <div className={`flex gap-1 mb-4 p-1 rounded-xl ${dark ? "bg-[#1e1e1e]" : "bg-[#e8e0d8]"}`}>
        {(["year","month","day"] as StatsView[]).map(v => (
          <button key={v} onClick={() => setView(v)}
            className={`flex-1 py-2 text-xs font-medium rounded-lg transition-all ${view === v ? "bg-[#B5845A] text-white shadow-sm" : dark ? "text-zinc-400 hover:text-zinc-200" : "text-zinc-500 hover:text-zinc-700"}`}>
...
export default function HabitTracker({ userEmail }: { userEmail: string }) {
  ...
  useEffect(() => {
    if (mounted) localStorage.setItem(STORAGE_THEME, theme);
  }, [theme, mounted]);
  ...
  async function addHabit(name: string, emoji: string, weeklyGoal: number) {
    const newHabit: Habit = { id: uid(), name, emoji, weeklyGoal, completedDays: [], createdAt: Date.now() };
  ...
  async function toggleToday(id: string) {
    ...
    setHabits(p => p.map(h => h.id !== id ? h : {
      ...h,
      completedDays: h.completedDays.includes(t)
        ? h.completedDays.filter(d => d !== t)
        : [...h.completedDays, t],
    }));
      ...
      <div className="max-w-3xl mx-auto px-4 py-6">
        ...
        <div className="flex items-start justify-between mb-5">
          <div>
            ...
            <p className={`text-xs mt-0.5 ${sub}`}>{userEmail}</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setTheme(dark ? "light" : "dark")}
              ...
              {dark
                ? <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M2 12h2M20 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42"/></svg>
            ...
            {habits.length > 0 ? (
              <div className={`${card} rounded-2xl mb-4`}>
                <div className={`flex items-center justify-between px-4 pt-4 pb-3 border-b ${div}`}>
                  ...
                  <span className={`text-xs ${sub}`}>{habits.length} {habits.length === 1 ? "привычка" : habits.length < 5 ? "привычки" : "привычек"}</span>
                </div>
                {habits.map((habit, idx) => {
                  ...
                  return (
                    <div key={habit.id} className={`flex items-center gap-3 px-4 py-3 ${!last ? `border-b ${div}` : ""}`}>
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl flex-shrink-0 ${dark ? "bg-[#2a2a2a]" : "bg-[#f4f0ec]"}`}>{habit.emoji}</div>
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-medium truncate ${txt}`}>{habit.name}</p>
                        <p className={`text-xs mt-0.5 ${sub}`}>{wc} из {habit.weeklyGoal} {habit.weeklyGoal === 7 ? "дней" : "раз"} на этой неделе</p>
              ...
              <div className="rounded-2xl p-5 mb-4 bg-[#B5845A]">
                ...
                <div className="flex items-center gap-4 mb-4">
                  <div className="relative w-20 h-20 flex-shrink-0 flex items-center justify-center">
                    <svg width="80" height="80" viewBox="0 0 80 80" className="-rotate-90">
                      ...
                      <circle cx="40" cy="40" r="32" fill="none" stroke="white" strokeWidth="6"
                        strokeDasharray={`${2*Math.PI*32}`} strokeDashoffset={`${2*Math.PI*32*(1-overallPct/100)}`}
                  ...
                  <div>
                    ...
                    {maxStreak > 0 && <p className="text-white/80 text-sm mt-1">🔥 Серия: <span className="font-semibold text-white">{maxStreak} {pluralDays(maxStreak)}</span> подряд</p>}
                    <p className="text-white/70 text-sm mt-0.5">Сегодня: {completedToday} / {habits.length}</p>
                  </div>
                </div>
                    ...
                    {weekDates.map((d, i) => {
                      ...
                      return (
                        <div key={d} className="flex-1 flex flex-col items-center gap-1">
                          <div className={`w-full aspect-square rounded-lg ${isToday && count > 0 ? "ring-2 ring-white/60" : ""}`}
